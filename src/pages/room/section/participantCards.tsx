...
export const ParticipantCards:React.FC<ParticipantCardsProps> = ({ list, show }) => {
  return (
    <div className={styles.participants}>
      { Object.keys(list).map(item => <ParticipantCard key={item} uuid={item} value={list[item]} show={show} />) }
    </div>
  )
}

const ParticipantCard: React.FC<ParticipantCardProps> = ({ uuid, value, ...restProps }) => {
